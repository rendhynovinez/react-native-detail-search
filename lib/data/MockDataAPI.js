...
export const getRecipesByCategoryName = async () => {
  // This one is not though which means it can't use await inside
  // return dispatch => {

  const URL = 'https://api.yelp.com/v3/businesses/search?location=new%20york&term=' + categoryName;
  ...
  axios.get(URL, { headers: { Authorization: AuthStr } }).then(response => {
    ...
    // alert(JSON.stringify(response.data.businesses));
    return response.data.businesses;
  }).catch(error => {
